...
export function GameBoard() {
  ...
  const loadNewImage = () => {
    ...
    // Affichage instantané : pas besoin d'attendre le chargement complet
    setTimeout(() => {
      setImageLoaded(true);
      setIsLoading(false);
    }, 100);
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white p-3 flex flex-col">
      <div className="max-w-md mx-auto w-full flex flex-col h-full">
        {/* Header avec score - Compact */}
        <div className="flex items-center justify-between mb-3 bg-black/30 backdrop-blur-sm rounded-xl p-3 border border-white/10">
          <div>
            <h1 className="text-2xl font-black bg-gradient-to-r from-yellow-400 to-orange-500 bg-clip-text text-transparent">
              CULTURESIPS
            </h1>
            <p className="text-xs text-slate-400">Devinez le pays !</p>
          </div>
          <div className="flex items-center gap-2 bg-orange-500/20 px-3 py-1.5 rounded-full border border-orange-500/30">
            <Beer className="w-5 h-5 text-orange-400" />
            <span className="text-xl font-black text-orange-400">{sips}</span>
          </div>
        </div>

        {/* Zone d'image - Ratio carré pour économiser l'espace */}
        <div className="mb-3 relative flex-1 flex items-center">
          <div className={`w-full bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl overflow-hidden shadow-2xl border-2 transition-all duration-300 ${
            showFeedback ? 'border-green-500 scale-105' : 'border-white/10'
          }`}>
            {isLoading || imageError ? (
              <div className="aspect-square bg-gradient-to-br from-slate-700 to-slate-800 flex flex-col items-center justify-center gap-3">
                  ...
                  <>
                    ...
                    {showFeedback && (
                      <div className="absolute inset-0 bg-green-500/30 flex items-center justify-center">
                        <Sparkles className="w-20 h-20 text-green-400 animate-pulse" />
                      </div>
                    )}
                  </>
                )}
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          {selectedCountries.map((countryCode) => (
            <button
              ...
              className="group relative bg-gradient-to-br from-red-500 to-rose-600 hover:from-red-600 hover:to-rose-700 disabled:from-slate-700 disabled:to-slate-800 p-4 rounded-xl flex flex-col items-center justify-center gap-1.5 transition-all duration-200 transform hover:scale-105 active:scale-95 disabled:scale-100 shadow-lg hover:shadow-2xl border-2 border-white/10 disabled:opacity-50"
            >
